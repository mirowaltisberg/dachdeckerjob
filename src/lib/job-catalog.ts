...
import { classifyDachdeckerTitle, isValidDachdeckerJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifyDachdeckerTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
function matchesQuery(job: JobListing, query: string): boolean {
  ...
  const hasPublicMatch = (queryToken: string) => {
    if (queryToken === "dach- und fassadenbau" || queryToken === "dachdecker") {
      return true;
    }
...
export async function getJobListingById(input: {
  ...
  if (!isValidDachdeckerJobId(input.id)) {
    return null;
  }
