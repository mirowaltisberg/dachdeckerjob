...
export function classifyDachdeckerTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-dachdecker-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-dachdecker-title" : "broad-role-with-dachdecker-context",
...
export function isValidDachdeckerJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getDachdeckerProfile(profile: JobSafetyProfile) {
