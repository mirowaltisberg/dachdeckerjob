...
import { classifyDachdeckerTitle, getDachdeckerProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifyDachdeckerTitle(input.title);
  if (
  ...
  const profile = getDachdeckerProfile(classification.profile);
