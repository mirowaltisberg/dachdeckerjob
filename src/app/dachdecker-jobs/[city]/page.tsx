...
import { DACH_CITIES, findDachCity } from "@/lib/dachdecker-cities";
...
export async function generateStaticParams() {
  return DACH_CITIES.map((city) => ({ city: city.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findDachCity(citySlug);
  if (!city) return { title: "Dachdecker Jobs" };

  const title = `Dachdecker Jobs ${city.name}`;
  const description = `Stelleninserate mit Dach- und Fassadenbaubezug für ${city.name} und Umgebung. Arbeitsort und Anforderungen im jeweiligen Inserat prüfen.`;
...
export default async function DachCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const city = findDachCity(citySlug);
  if (!city) notFound();

  const result = await searchJobListings({
    q: "Dach- und Fassadenbau",
    loc: city.name,
    limit: 18,
    offset: 0,
    sort: "newest",
      ...
      <main id="main-content" className="bg-background">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            ...
            <p className="text-slate-600 text-lg leading-relaxed mb-4 max-w-3xl">
              Diese Seite zeigt Suchtreffer mit Dach- und Fassadenbaubezug für {city.name}.
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Dach- und Fassadenbau-Berufsbezeichnungen in der Suche
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 pb-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            {DACH_CITIES.filter((item) => item.slug !== city.slug).map((item) => (
              <Link
                key={item.slug}
                href={`/dachdecker-jobs/${item.slug}`}
