...
const homepageMetadata: Metadata = {
  ...
  description:
    "Finde Stellenangebote für Dachdecker, Flachdach- und Fassadenfachkräfte sowie Projektleitung Gebäudehülle in der Schweiz.",
...
export default async function HomePage({ searchParams }: HomePageProps) {
  ...
  const heroContent = (
    <div className="trade-hero-grid">
      <div>
        <p className="trade-kicker">Steildach · Flachdach · Gebäudehülle</p>
        <h1 className="trade-display">Dachdeckerjobs.<em>Über der Traufe.</em></h1>
        <p className="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-lg">Dach- und Gebäudehüllenstellen — nach Fachgebiet, Ort und Pensum.</p>
      </div>
      <div className="roof-section" aria-hidden="true"><span className="roof-section__label">Schnitt 01 / First bis Traufe</span></div>
    </div>
  );
  ...
  return (
    <>
      ...
      {Object.keys(rawSearchParams).length === 0 ? (
        <HomepageLanding initialData={initialData} config={{ hero: heroContent, sectionKicker: "Dachjournal", sectionTitle: "Aktuelle Dachstellen", keywordPlaceholder: "Welchen Job suchst du?" }} />
      ) : (
