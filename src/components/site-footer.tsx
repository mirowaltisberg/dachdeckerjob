    ...
    <footer className="trade-footer pt-14 pb-8">
      <div className="container mx-auto max-w-6xl px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.15fr_1fr_1fr_1fr] gap-9 lg:gap-12 mb-12">
          ...
          {/* Canton navigation — each links to the Dachdecker page for that canton */}
          <nav aria-label="Kantone">
            ...
            <ul className="space-y-1.5">
              {FOOTER_CANTONS.map((canton) => (
                <li key={canton.key}>
                  <Link
                    href={`/dachdeckerjobs/${toRoleSlug(DEFAULT_ROLE)}/${toCantonSlug(canton.key)}`}
                    className="text-sm transition-colors duration-150"
                  >
                    Dachdeckerjobs {canton.label}
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <div className="border-t border-sidebar-border pt-6 mt-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-sm">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <p>© {new Date().getFullYear()} dachdeckerjobs.ch — Alle Rechte vorbehalten.</p>
            <nav aria-label="Offizielle Quellen" className="flex items-center gap-4">
              <a href="https://www.berufsberatung.ch/de/berufe/dachdecker-in-efz" target="_blank" rel="noopener noreferrer" className="transition-colors">berufsberatung.ch</a>
